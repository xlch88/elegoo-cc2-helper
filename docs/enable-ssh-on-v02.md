# 如何在 v02.01.00.00 固件上开启SSH

elegoo官方在v02.xx.xx固件中移除了SSH功能，导致很多用户无法通过SSH远程连接打印机进行调试和操作。本文将介绍如何在v02.01.00.00固件上重新开启SSH功能。

通过对 v02.xx.xx 固件上传流程的分析，可以借助局域网文件上传接口补全 SSH 相关组件，从而重新开启 SSH 功能。
...
此功能需要启用LAN mode（仅局域网模式）。在开启SSH后您可以关闭仅局域网模式以正常使用云功能。

具体需要在机器屏幕上进行设置。

第一步：点击设置图标 -> 网络，连接WiFi。

第二步：点击设置 -> 下滑找到“仅局域网”，点击进入，然后开启“仅局域网”模式。

第三步：开启访问码，并点击刷新按钮生成新的访问码。记录下屏幕上显示的IP地址和访问码。
...
因此，实际请求里不要直接写 `/tmp/test.txt`，而是把 `../../tmp/test.txt` 做 URL 编码后放到 `X-File-Name` 中，例如：
...
上传请求不是 `multipart/form-data`，而是把文件内容直接作为 HTTP POST body 发送到：

```text
POST http://打印机IP/upload/udisk
```
...
| `X-File-MD5`     | POST body 的 MD5                                 |
| `Content-Range`  | 文件范围，格式为 `bytes 0-(文件大小-1)/文件大小` |
| `Content-Type`   | `application/octet-stream`                       |
| `Content-Length` | 文件大小                                         |

例如上传 1 字节内容 `1` 到 `/1.txt`，请求大致如下：

```bash
printf '1' | curl -i -X POST 'http://打印机IP/upload/udisk' \
  -H 'X-Token: 屏幕上的访问码' \
  -H 'X-File-Name: ..%2F..%2F1.txt' \
  -H 'X-File-MD5: c4ca4238a0b923820dcc509a6f75849b' \
  -H 'Content-Range: bytes 0-0/1' \
...
v02 固件移除 SSH 相关文件后，可以参考 v01 固件中的 OpenSSH server 组件，将缺失文件补回设备根文件系统。当前示例的 `files` 目录会被映射到设备根目录，例如 `files/usr/sbin/sshd` 对应设备上的 `/usr/sbin/sshd`。
